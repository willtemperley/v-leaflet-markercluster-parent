    ...
    L.MarkerClusterGroup = L.FeatureGroup.extend({
        options: {
            ...
            if (e instanceof L.LayerGroup) {
                var t = [];
                for (var i in e._layers) t.push(e._layers[i]);
            ...
            var n = e, s = this._map.getZoom();
            if (e.__parent) for (; n.__parent._zoom >= s;) n = n.__parent;
            return this._currentShownBounds.contains(n.getLatLng()) && (this.options.animateAddingMarkers ? this._animationAddLayer(e, n) : this._animationAddLayerNonAnimated(e, n)), this
            ...
            var t, i, n, s, r = this._featureGroup, o = this._nonPointGroup, a = this.options.chunkedLoading,
                h = this.options.chunkInterval, u = this.options.chunkProgress;
            if (this._map) {
                var l = 0, _ = (new Date).getTime(), d = L.bind(function () {
                    for (var t = (new Date).getTime(); l < e.length; l++) {
                        ...
                        if (s = e[l], s.getLatLng) {
                            if (!this.hasLayer(s) && (this._addLayer(s, this._maxZoom), s.__parent && 2 === s.__parent.getChildCount())) {
                                var n = s.__parent.getAllChildMarkers(), c = n[0] === s ? n[1] : n[0];
                                r.removeLayer(c)
                            }
                        } else o.addLayer(s)
                    }
                ...
                for (t = [], i = 0, n = e.length; n > i; i++) s = e[i], s.getLatLng ? this.hasLayer(s) || t.push(s) : o.addLayer(s);
                this._needsClustering = this._needsClustering.concat(t)
            }
            return this
        }, removeLayers: function (e) {
            var t, i, n, s = this._featureGroup, r = this._nonPointGroup;
            if (!this._map) {
                for (t = 0, i = e.length; i > t; t++) n = e[t], this._arraySplice(this._needsClustering, n), r.removeLayer(n), this.hasLayer(n) && this._needsRemoving.push(n);
                return this
            }
            if (this._unspiderfy) for (this._unspiderfy(), t = 0, i = e.length; i > t; t++) n = e[t], this._unspiderfyLayer(n);
            for (t = 0, i = e.length; i > t; t++) n = e[t], n.__parent ? (this._removeLayer(n, !0, !0), s.hasLayer(n) && (s.removeLayer(n), n.clusterShow && n.clusterShow())) : r.removeLayer(n);
